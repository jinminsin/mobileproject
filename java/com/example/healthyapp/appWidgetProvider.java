        ...
        public void run() {
                ...
                if (character.getCharacter() == 1) { //캐릭터 타입 1
                    if (character.getLevel().getLevel() <= 5) { //조건 //지금은 2초단위로 변경되도록 해놨음. 레벨단위로 수정필요. 레벨은 data2
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.a1);
                    } else if (character.getLevel().getLevel() <= 10) {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.a2);
                    } else {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.a3);
                    }
                } else if (character.getCharacter() == 2) { //캐릭터 타입 2
                    if (character.getLevel().getLevel() <= 5) { //조건 //지금은 2초단위로 변경되도록 해놨음. 레벨단위로 수정필요. 레벨은 data2
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.b1);
                    } else if (character.getLevel().getLevel() <= 10) {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.b2);
                    } else {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.b3);
                    }
                } else { //캐릭터 타입 3
                    if (character.getLevel().getLevel() <= 5) { //조건 //지금은 2초단위로 변경되도록 해놨음. 레벨단위로 수정필요. 레벨은 data2
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.c1);
                    } else if (character.getLevel().getLevel() <= 10) {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.c2);
                    } else {
                        views.setImageViewResource(R.id.w_imageView1, R.drawable.c3);
                    }
                }
